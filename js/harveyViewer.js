...
function build_map(){ // uses leaflet.js to build a zoomable map
    ...
    var map = L.map('map').setView([30.315,-95.185],8);
...
function add_points_to_map(data,map){
    ...
    dateArray.forEach(function(ts){
        if (data.time_series.interval_data[ts].hasOwnProperty("geo_tagged_native_media")){
            data.time_series.interval_data[ts].geo_tagged_native_media.forEach(function(feature){
...
function update_scales(sizes){
    ...
    var x = d3.time.scale()
        .domain([new Date("2017-08-22 06:00:00"), new Date("2017-09-12 06:00:00")])
...
function create_timeseries_plot(series1, series2, sizes){
    ...
    brush = d3.svg.brush()
        .x(x)
        .extent([new Date("2017-08-22 06:00:00"), new Date("2017-08-22 12:00:00")])
        .on("brush", function() {
            brushed(series1);
        });
...
function animate() {
    ...
    var dataSetEndTime = new Date("2017-08-22 12:00:00"); //TODO: this needs to be 'dynamic'.
...
d3.json("./data/event_viewer.json", function(collection) { //Load JSON dataset
    ...
    var timeseries2 = convert_to_array(collection,"geo_tagged_native_media");
